...
class Estat(object):
    ...
    def copia(self) -> Estat:
        ...
        for furgoneta in self.furgonetes_util:
            ...
            furgoneta_copia = [[furgoneta[0][0], furgoneta[0][1]], [], furgoneta[2]]
            
            # Recorre los destinos de la furgoneta actual y crea copias de los destinos
            for desti in furgoneta[1]:
                destino_copia = desti[:]
                furgoneta_copia[1].append(destino_copia)
    ...
    def benefici(self, id_furgoneta: int, new_state: Estat) -> float:
        ...
        for parada in range(len(new_state.furgonetes_util[id_furgoneta][1])):
            ...
            if parada==0: #estem realitzant el primer transport (1r desti)
                ...
                #c1=abs(self.cost_transport(bicis_transportades, estacio_origen, desti_estacio))
            else:
                estacio_primer_desti = new_state.furgonetes_util[id_furgoneta][1][parada-1][0]
                #c2=abs(self.cost_transport(bicis_transportades-bicis_deixades,estacio_primer_desti, desti_estacio))
